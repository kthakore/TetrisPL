...
sub notify {
    ...
    if (defined $event and $event->name eq 'Tick') {
            ...
            || (defined $self->{key} && $self->{key} =~ 'down'))
        {
            ...
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'Quit' )
              if $key =~ 'escape';
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'CharactorMoveRequest', direction => $self->ROTATE_C)
              if $key =~ 'up';
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'CharactorMoveRequest', direction => $self->ROTATE_CC)
              if $key =~ 'space';
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'CharactorMoveRequest', direction => $self->DIRECTION_DOWN)
              if $key =~ 'down';
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'CharactorMoveRequest', direction => $self->DIRECTION_LEFT)
              if $key =~ 'left';
            $event_to_process = SDL::Tutorial::Tetris::Event->new( name => 'CharactorMoveRequest', direction => $self->DIRECTION_RIGHT)
              if $key =~ 'right';
        }
